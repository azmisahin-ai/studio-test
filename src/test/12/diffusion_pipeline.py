    ...
    def load_pipeline_by_id(self):
        ...
        if self.model_id in CustomDiffusionPipeline._loaded_models:
            ...
            if self.model_id == "stabilityai/stable-diffusion-xl-base-1.0":
                ...
                self.pipe = StableDiffusionPipeline.from_pretrained(self.model_id, scheduler=scheduler, torch_dtype=torch.float16)
            elif self.model_id == "stabilityai/sdxl-turbo":
                # 6
                self.pipe = AutoPipelineForText2Image.from_pretrained(self.model_id, torch_dtype=torch.float16, variant="fp16")
                ...
                # 9
                self.pipe = StableDiffusionXLPipeline.from_pretrained(self.model_id, torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
            elif self.model_id == "cagliostrolab/animagine-xl-3.0":
                # 10
                # Load VAE component
                vae = AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix")
                self.pipe = StableDiffusionXLPipeline.from_pretrained(self.model_id, vae=vae, torch_dtype=torch.float16,  use_safetensors=True)
